...
return view.extend({
	...
	render: function(data) {
		...
		o = s.taboption('setup', form.ListValue, 'upnp_igd_compat', _('UPnP IGD compatibility'),
			_('Set compatibility mode (act as device) to workaround IGDv2-incompatible clients; %s only work with %s (or) <br>Emulate/report a specific/different device to workaround/support/handle/bypass/assist/mitigate... (alternative text welcome)').format('PlayStation, CoD', 'IGDv1'));
		...
		s = m.section(form.GridSection, 'internal_network', _('Enable Networks / Access Control'),
			...
			_('Use an access control preset to define which ports all devices on a network can map.') + ' ' +
		...
		o = s.option(form.ListValue, 'access_preset', _('Access preset'),
			_('Select an access control preset for ports that all devices on this network can map'));
		o.value('none', _('None / accept extra ports only'));
		...
		o = s.option(form.Value, 'accept_ports', _('Extra ports'),
			_('Accept these ports or port ranges on this network as well'));
		o.retain = true;
		o.validate = function(section_id, value) {
		...
		o = s.option(form.Value, 'reject_ports', _('Reject ports'),
			_('Reject unsafe/insecure/risky FTP/Telnet/DCE/NetBIOS/SMB/RDP ports on this network by default, override other settings'));
		...
		o = s.option(form.Flag, 'acl_before', _('ACL before'),
			...
			_('Sequence: 1. Reject ports, 2. ACL entries (if enabled), 3. Preset ports, 4. Extra ports'));
		o.editable = true;
		o.retain = true;
